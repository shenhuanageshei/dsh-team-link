...
| 交付 `detail` **改后** | **345 码点 / 6 行** ✓（≤ 350 且 ≤ 6 行）|
### 病灶（本批，修复前必红，实测读数）

| 套件 | 判据 | 修复前（本批新测试 ＋ `HEAD` 实现）|
| --- | --- | --- |
| 宿主 | **U33c 参照夹具 ≤6 行 / ≤350 码点**（**新增**）| `{"cp":569,"lines":10}` —— 超 **219** 码点 / 超 **4** 行 |
...
| 宿主 | 既有断言 8 条（措辞被删 / 被改）| U16 确认框 2 条 · U33 换槽位 1 条 · 长 cwd 1 条 · 长团队名 1 条 · 两者都长 1 条 · U30 成本行 1 条 · DEFECT-4 ② 1 条 · 裁定 2 ② 1 条 |

**合计 `966 (failed: 15)`**；客户端 `269 (failed: 0)`（`lib/client.js` 本批一字未改 ⇒ 红相不该动它，实测也确实没动）。
...
### 全夹具逐用例读数（本批，**改前 → 改后**；两列都是同一具夹具、两棵实现上实跑）

| 用例 | 改前（cp / 行）| 改后（cp / 行）|
| --- | --- | --- |
| **参照夹具**（12 码点协调者 id）| 569 / 10 | **345 / 6** |
| 参照夹具（28 码点协调者 id，= 真机 585 那一次）| 585 / 10 | 361 / 6 |
| 常规 `n=2 roles=worker-a,worker-b task=做接口`（`team=night-shift`）| 581 / 10 | 330 / 6 |
| 8 会话 ＋ 3000 字正文 | 547 / 10 | 408 / 6 |
| 39 字团队名 | 505 / 10 | 375 / 6 |
| 110 字 cwd | 594 / 10 | 376 / 6 |
| 长团队名 ＋ 长 cwd ＋ 8 会话 ＋ 500 字正文 | 565 / 10 | 426 / 6 |
| DEFECT-4 槽位 `n=2 team=title-dialog roles=worker-a,worker-b` | 587 / 10 | 367 / 6 |
| 既无正文也无 `task=`（`team=t n=2`）| 554 / 10 | 356 / 6 |
| 300 字 `model=` | 534 / 10 | 402 / 6 |
| 60 字 `preset=` | 513 / 10 | 364 / 6 |
| **(i) 中间档夹具** | 594 / 10（body 582 ＋ 全档标注 12）| **496 / 6**（逐字 = 必备 body）|
| **(ii) 第二具中间档夹具** | 604 / 10（body 592 ＋ 标注 12）| **506 / 6**（逐字 = 必备 body）|
| **(iii) 机制一夹具** | 705 / 10（body 693 ＋ 标注 12）| **607 / 6**（逐字 = 必备 body，**仍 > 600**）|

**读法**：**十四具全部收到 6 行**；除机制一那一档（必备 body 607 > 600、**永不裁剪** ⇒ 设计档 §10.2.8.4 残余行「第一机制」原样保留）外**全部 ≤ 430 码点**，离硬顶 600/12 更远。机制一是**唯一**仍可能超 600 的一档，而它超的原因是**必备披露本身**，不是本批的措辞。
