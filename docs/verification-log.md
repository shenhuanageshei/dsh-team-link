...
## 可观测批（A 批）：读窗扩展 · 团队状态卡 · 台账收件视图 ＋ 派生回执

> 设计（唯一事实源）：[observability-batch-design-2026-09-26.md](observability-batch-design-2026-09-26.md)（§4.1 读窗 / §4.2 状态卡 / §4.3 收件视图 / §4.4 派生回执 / §5 红线五条 / §6 判据 U1–U14 / §7 明确不做 / §8 假设 A1–A5）。变更史见 [../CHANGELOG.md](../CHANGELOG.md) 的「可观测批（A 批）」。

### 本批做了什么（三件交付）

1. **读窗扩展**：`team_link_list_sessions` 新增**可选且互斥**的 `readIds`（点名读，去重后 ≤12，超出 / 未知 id 一律**拒绝且零读**，点名优先占额、其余按原顺序补足）与 `offset`（分页，整数、越界拒绝并给有效区间）；末尾追加「未读 Y 行 ＋ 怎么读」提示段（**默认调用也带**，本次唯一的格式新增）；
2. **`team_link_status`（新工具，`timeoutMs` 30000）**：六段只读状态卡 + 反面预警注记（判据写死：`N == 0 且 M > 0` 才打）；
3. **`team_link_team_read` 新块**：收件视图（按 `ref=t-<n>` 分组）+ 派生回执（三态 ✅ / ⚠ / 未读）。

**成本不变量按工具分账**：`list_sessions` 三条路径 ≤ 12 · `team_read` ≤ `min(12, 1+|readIds|)`（自读占 1 个额）· `status` ≤ 12。

### 红相（本批 60 条判据 ＋ **实施前**的 `lib/index.js`）

方法（安全还原纪律）：把工作树版 `lib/index.js` **读进内存** → 用 `git show HEAD:lib/index.js`（`89074fe`）覆写 → 跑 `node host-half.test.mjs` → **还原内存里的那份** → **sha256 逐字节复核**。**不留备份文件、不留探针**，工作树零残留。

- sha256（实施版）：`fde1b78573f88d392cdf3666c38bf1f70132439209666d590bfc46bf546ff7c1`，**还原后逐字节一致**（`MATCH`）；
- 读数：**`assertion total: 1117 (failed: 55)`**（`nodeExit=1`，`assertion total` 照常打印 —— Y7 纪律：新判据在旧实现上**报 FAIL 而不是把整轮打崩**，办法是三个 Y7-safe 调用器 `listCall` / `statusCall` / `readCall`，把「参数不存在 / 工具未注册」的抛错收成一段文本）；
- **本批新断言 60 条，其中 54 条在实施前实测红**；另 **6 条在两种实现上都通过**，逐条如实列出（它们是**刻意**的回归锁，或红相上 trivially 成立）：

| # | 在红相上也通过的 6 条 | 为什么 |
| --- | --- | --- |
| 1 | U1 默认调用：读窗就是前 12 行（surfaceReads 恰 12 个且逐个按序） | **回归锁**：默认路径不许变 |
| 2 | U1 默认调用：每行结构逐字保持（id — 代理状态 · 创建于 … （读数 …，>2min 作废）） | **回归锁**：既有行格式逐字不变 |
| 3 | U1 默认调用：窗口内 12 行给真实 verdict，窗口外 2 行标未读 | **回归锁**：既有未读口径不变 |
| 4 | U12 零回归：派生读数（最后主张 / 未消解存疑）与原始行窗口一字未动 | **回归锁**：既有四块不许动 |
| 5 | U3 边界：点名的 id 本就在默认窗内 → 只占一个额、不重复读 | 旧实现**忽略未知参数**（`defineTool` 容许未声明的额外参数），于是它读的仍是默认窗 —— 这条断言的正是「窗口内容不变」，故两版都绿 |
| 6 | U9 零写入：调用前后 settings 命名空间逐字节不变 | 红相上 trivially 成立（旧实现没有这个工具，自然不写设置）；绿相上它是真锁（另一半「磁盘 tasks.md 逐字节不变」在红相上**是红的**） |

- **另有 1 条既有断言在红相上变红**：`§11.9 文档面=实现面（工具计数同改锁）`——临时状态下 README 已写 10 个工具而旧实现只注册 9 个。**这正是这条锁该红的原因**（加工具而不同改文档即红），不是回归。

| 阶段 | 新断言 | 红相红 | 绿相绿 |
| --- | --- | --- | --- |
| 1 读窗（U1–U7 的 list 半边） | 23 | 19 | 23 |
| 2 状态卡（U8 / U9 / U13 / U7 的 status 半边） | 20 | 19 | 20 |
| 3 收件视图 ＋ 派生回执（U10 / U11 / U14 / U12 / U7 的 team_read 半边） | 17 | 16 | 17 |
| **合计** | **60** | **54** | **60** |

### 绿相（本批最终读数，实跑两行原文）

```text
node host-half.test.mjs    → ALL PASS / assertion total: 1117 (failed: 0)
node client-half.test.mjs  → ALL PASS / assertion total: 269 (failed: 0)
```

基线 `1057` / `269` ⇒ 新增 **60** 条宿主断言（`1117 = 1057 + 60`）；**浏览器半边本批一字未改**（仍 269 —— 设计 §7 ① 明确不做客户端面板渲染）。

### §8 假设 A1–A5 的实测结论（实施期验证，如实标注）

| # | 假设 | 实测结论 |
| --- | --- | --- |
| A1 | `ctx.sessionQuery.readSurface(selfId)` 对**调用方自己**可读 | **harness 内成立**：`team_read` 的自读走的正是列表工具一直在用的同一个 `readSurface` 调用面（无结构差异），U10/U14 的三态全部由它派生；**真机（DSH 进程内）未复验** —— 如实标注为「待宿主复验」，不当作已证 |
| A2 | 点名读 12 个仍在预算内（`list_sessions` 60s / `status` 30s） | **读数已锁死**：三工具的 surface 读数上限逐条断言（12 / 12 / 12，含 15 个点名时的 1+11=12）；**墙钟未实测**（需真机跑真实 26 会话工作区），`timeoutMs` 常量一字未动（60s / 30s / 30s） |
| A3 | 消息的 `ref` 在 surface 事件里可稳定抽取（banner 首行格式固定） | **成立**（对 §3.4 banner 形状的样本）：`ref=t-<n>` 从既有事件文本里稳定抽出并按任务号分组；`ref` 是别的值（如旧样式的 `slp-…`）**不归组、只计数**（已断言）—— 这是有意的归组面收窄 |
| A4 | 状态卡输出长度有界 | **成立**：六段 + 活性 12 行（超出如实标注）+ 台账尾 `tasksTail`（默认 5、上限 20，超出**拒绝**）+ 会话面 ≤ `LIST_LIMIT`(50)；**未做真机像素级「一屏」测量**（界是常量与拒绝路径，不是渲染尺寸） |
| A5 | 读窗内的 surface 事件带可用的时间戳 | **成立**：注记的锚 `T0` 取自真实事件的 `time`；三态（`N==0 且 M>0` 打 / 其余不打 / 无时间戳时**如实标「无法计算」**）都有断言 —— **订正**：第三态（无时间戳）的那条断言当时**并不存在**（套件里 grep `无法计算` = **0 命中**，本行原先的「都有断言」不实），已由 **2026-09-27 的分歧审计修复轮 D1** 补齐；逐条读数见本档「分歧审计修复轮（DIVERGENCE(12)）」一节 |

### 边界证据（逐条取值，全部走真工具）

| # | 实测原文 |
| --- | --- |
| **U4** | `列出会话失败：readIds 最多 12 个（去重后 13 个）——超出即拒绝，本次零读；请分两批点名，或用 offset 分页走完列表。`（该次 surface 读数 **0**） |
| **U5** | `列出会话失败：readIds 里的 session-not-in-this-list 不属于本次列表（本工作区的会话列表里没有它）——先用 team_link_list_sessions 复制正确的 id。`（读数 **0**） |
| **U6** | `readIds（点名读）与 offset（分页）互斥…` / `offset 必须是整数（收到 1.5）。` / `offset 越界：本次列表共 14 行，有效区间 0..13（offset=k 表示从第 k+1 行起读 12 行）。`（三类拒绝**读数全为 0**） |
| **U1 尾巴** | `读窗：默认前 12 行` ＋ `未读 2 行（读窗 12/共 14 行）：读第 13–24 行 → offset=12；或点名 → readIds=["session-rw-12", "session-rw-13"]`（提示段在**最后一行之后**） |
| **U3** | `读窗：点名读 1 个（session-rw-13）+ 默认窗补足`；读数 = 被点名的那个 ＋ 前 11 行，不重复读 |
| **U13** | 触发：`⚠ 窗口内 0 条消息 / 台账同期新增 7 行 —— 会话可能已停止说话只写行（读数，不是裁决）。`；`N>0` 与 `M==0` 两种情形**连那一段标题都不出现**；无时间戳时 `（反面预警注记无法计算：本读窗内没有带时间戳的 surface 消息——不猜。）` |
| **U14** | `- t-7 → session-worker-b（MM-DD HH:mm）：✅ 有反应（MM-DD HH:mm 起有 assistant 事件）` · `：⚠ 无后续反应（该时刻之后无 assistant 事件）` · `：未读（未点名读该会话；要读 → readIds=["session-worker-d"]）` · `：未读（点名的会话面读取失败——不用空数据算读数）` · 非 delivered 的那笔如实写 `：未投递（outcome=refused）` |
| **U8 状态卡** | 六段标题逐字在场；`token tok-1a2b…7890（掩码…）` 且**完整令牌零命中**；`session-st-12：未读（本卡只读了前 12 行）…`；`tasksTail 越界（收到 21）：有效区间 1..20，默认 5。` |
| **U9** | 调用前后 `JSON.stringify(env.ns.data)` 与 `tasks.md` 的 sha256 **都不变**（连换届过期清扫都不跑） |
| **U11** | 未点名时 `query.surfaceReads` 恰 `["session-self"]`（**目标面零读**）；点名 3 个 ⇒ 恰 4 次（`min(12, 1+3)`）；点名 15 个 ⇒ 恰 12 次，超额的 4 个逐个标「本次未读」 |

### 计数与同批同步

- `README.md`：§二 新增「读窗参数 ＋ 成本不变量」表 · 新增「团队状态卡（只读）」一段（含目录条目）· §四 新增「收件视图与派生回执」小节 · 工具一览表三行（list / team_read / **新工具 status**）· 设计文档索引新增本档一行 · **三处读数**（tests 徽章 · Agent Teams 对照表稳定性格 · §十「当前读数」）`1057 → 1117`；
...
> 变更史见 [../CHANGELOG.md](../CHANGELOG.md) 的同名条目。**本轮是修复轮**：只闭环只读分歧审计的 12 条结论（D1–D12），**不夹带新功能、不扩范围**；A 批设计 §7「明确不做」继续有效，B 档（`docs/team-mode-batch-design-2026-09-26.md`）**一字未动**。

### 12 条分歧的处置（逐条）

| # | 级别 | 分歧 | 处置 | 落点 |
| --- | --- | --- | --- | --- |
| **D1** | 🔴 | 设计 §8.1 A5 与账本都说「三态（触发 / 不触发 / 无时间戳「无法计算」）**都有断言**」，实际 `grep 无法计算 host-half.test.mjs` = **0 命中**（实现侧有那句话，判据没有） | **补那条断言**（无时间戳 ⇒ 输出「无法计算」且**不打**计数注记），并把两处声明订正为「第三态由本轮 D1 补齐」 | `host-half.test.mjs`（新断言）· 设计 §8.1 A5 · 本档 A 批一节的 A5 行 |
| **D2** | 🟡 | 无锚时的口径未进设计（`computed:false` 的「无法计算」句在「读窗非空但无带时间戳消息」时会照打），聚合态 `M` 未定义 | **保留实现行为、把口径写进设计 §4.2**（三态逐条 + ⑤ 聚合态 `M` = 所示各团队之和）；**补两条断言**覆盖 ①（读窗为空 ⇒ 整段不打印）②（无时间戳 ⇒ 打「无法计算」句） | 设计 §4.2 · `host-half.test.mjs`（2 条断言） |
| **D3** | 🟡 | `team_read` 的 `readIds` 边界（去重后 ≤11；超出**截断并标注未读**而非拒绝）没进设计的边界表 | **实现不动**；设计 §4.1 补「点名读上限与超限行为」表（两个工具两种形态同一条不变量），§4.3 指明该行为 | 设计 §4.1 / §4.3 |
| **D4** | 🟡 | 派生回执的投递事实来源与窗口未进设计（`tool/call` + `tool/ptc-dispatch`、`callId` 配对、窗口单位是「最近 20 次 send」） | 写进设计 §4.4；**补四条断言**（ptc-dispatch 载体 / 实参兜底解析 `team:<name>/<role>` / 回执截断行 / 行上限） | 设计 §4.4 · `host-half.test.mjs`（4 条断言） |
| **D5** | 🟡 | `team_read` 的 `readIds` 非数组拒绝、以及 D4 列的四个分支**全无断言** | **补断言**（逐分支一条；非数组 ⇒ 拒绝且**零读**） | `host-half.test.mjs`（含在 D4 的 4 条内 + 非数组 1 条） |
| **D6** | 🟡 | README 设计文档索引仍写「形态开关等第 2–5 步待实施」，父档 §7 已把步 2 / 4 / 6 标 ✅ | 改 README 那行为「第 2 / 4 / 6 步已随 A 批实施；第 3 步（形态开关）与第 5 步（teammate 交卷提示）待实施」 | `README.md` 设计文档索引 |
| **D7** | 🟡 | 注释承诺行上会标注「按当前名册解析」，`renderReceiptRow` 却只特判 `回执截断` ⇒ 换届后可能把 ✅/⚠ 判给当时并非收件人的会话且**不告知** | **实现那个标记**（按当前名册解析出来的行如实标注），并补一条断言 | `lib/index.js`（新常量 `ROSTER_RESOLVED_NOTE` + `receiptRowsOf` / `renderReceiptRow` + 工具描述）· `host-half.test.mjs` |
| **D8** | 🟡 | 第 4 态「：未投递（outcome=…）」实现里有、账本里有实测原文，设计 §4.4 与 README 都只写三态 | 设计 §4.4（判据 + 示例 + U14 行）与 README 的四态表各补第 4 态（措辞只陈述 `outcome`，不下结论） | 设计 §4.4 · `README.md` |
| **D9** | 🟡 | 断言半边**恒真**（`surfaceReads.slice(length).length === 0`），U13 的「注记零额外读」那半边是空的 | 换成**真实计数**：打注记那一次与不打注记那一次，各自的 surface 读数逐次相等（各恰 12 = 同一个有界读窗） | `host-half.test.mjs` |
| **D10** | 🟡 | 聚合态台账尾实为**每团队一次文件读**，设计 §4.2 成本列写「一次文件读」（README 本来就对）；聚合 M 与聚合上限无断言 | 设计 §4.2 成本列回填为「单团队 1 次 / 聚合 N 次（N = 所示团队数）」；**补一条聚合上限断言**（`M` = 3 + 2 = 5 之和，两个团队的台账尾都在卡上） | 设计 §4.2 · `host-half.test.mjs`（1 条断言） |
| **D11** | 🔵 | 设计 §4.2 写「`ctx.agents` + `ctx.sessions.list` 快照」，模块级 `inject` 里没有 `sessions` | 改设计那一行为 `ctx.sessionQuery.listSessions`（**与 `list_sessions` 同源同序**）+ `ctx.agents.get(id)` | 设计 §4.2 |
| **D12** | 🔵 | 归组面宽于假设 A3：`ref=(t-\d+)` 对**整段文本** exec（不要求 banner、不排除本会话自己），未进设计 | 写进设计 §4.3；**补一条断言**（**非 banner** 文本也归组，且发送方回落标「（本会话）」） | 设计 §4.3 · `host-half.test.mjs`（1 条断言） |

### 本轮新增 10 条断言与红相

- **命令**：`node host-half.test.mjs && node client-half.test.mjs`（本仓纪律：不用 `node --test`，沙箱下 `spawn EPERM`）；
- **绿相（工作树，实跑两行原文）**：

```text
node host-half.test.mjs    → ALL PASS / assertion total: 1127 (failed: 0)
node client-half.test.mjs  → ALL PASS / assertion total: 269 (failed: 0)
```

  基线 `1117` / `269` ⇒ **新增 10 条宿主断言**（`1127 = 1117 + 10`）；**浏览器半边本轮一字未改**（仍 `269`）。
- **红相 A（「本轮修复前」的实现 = 工作树，但 D7 那个标记还没落码）**：`assertion total: 1127 (failed: 1)` —— 唯一的一条红是 **D7**（`FAIL  D7 读数不失准: …按当前名册解析…`），其余 9 条**在两种实现上都绿**（见下「负相如实标注」）；
- **红相 B（A 批实施前的 `89074fe`）**：`assertion total: 1127 (failed: 65)` = A 批的 **55** 条（54 新 ＋ 1 既有工具计数锁，与本档「可观测批（A 批）」一节逐条相符）＋ **本轮 10 条全红**（旧实现里既没有 `team_link_status`，`team_read` 也没有 `readIds` 参数）；
- **红相 B 的安全方法（与 A 批同一纪律）**：把工作树版 `lib/index.js` **读进内存** → 用 `git show HEAD:lib/index.js` 覆写 → 跑 `node host-half.test.mjs` → **还原内存里的那份** → **sha256 逐字节复核**（`restoreMatch: true`；本轮工作树版 sha256 `d6ae2f78c1d6c13db8fb2d8ad0cc56cfd623702f7a675675d69c074211b9ab1a`）。**不留备份文件、不留探针**，工作树零残留。

| # | 新断言 | 本轮修复前（工作树） | `89074fe`（A 批实施前） |
| --- | --- | --- | --- |
| 1 | D1/A5 注记第三态：无时间戳 ⇒ 打「无法计算」句、**不打**计数注记 | 绿（**负相**） | 红 |
| 2 | D2① 读窗为空 ⇒ 注记**整段不打印**（连标题都不出现） | 绿（**负相**） | 红 |
| 3 | D10 聚合口径：`M` = 所示各团队之和（3 + 2 = 5） | 绿（**负相**） | 红 |
| 4 | D12 归组面：非 banner 文本同样归组 + 发送方回落「（本会话）」 | 绿（**负相**） | 红 |
| 5 | D4 载体：`tool/ptc-dispatch` 那一类投递照样成行 | 绿（**负相**） | 红 |
| 6 | D4 实参兜底：按实参解析 `team:<name>/<role>`，表达式与现任一并显示 | 绿（**负相**） | 红 |
| 7 | D4 回执截断行：只报告回执自己报的总数，不编目标身份 | 绿（**负相**） | 红 |
| 8 | D4 行上限：恰 20 行 + 一行「本次还有 N 行未显示」（N = 真实差额） | 绿（**负相**） | 红 |
| 9 | D5 `readIds` 非数组 ⇒ 拒绝且**零读** | 绿（**负相**） | 红 |
| 10 | **D7** 按当前名册解析出来的行如实标注（字面 id 的行不带） | **红**（`failed: 1`） | 红 |

**负相如实标注（不冒充「修复前必红」）**：上面 9 条在「本轮修复前」**本来就绿** —— 它们补的是**判据覆盖缺口**（旧实现没有这个 bug，只是没有这条锁），D1 那条尤其是本轮审计的**原点**（账本原先把该文案当实测原文贴出，而套件里零命中）。这与本档既有记法（`★` 标负相）同源；本轮**唯一的真红相**是 D7。

### D1–D12 逐条「怎么验证它没了」

| # | 一行验证 |
| --- | --- |
| D1 | `grep -c 无法计算 host-half.test.mjs`：修复前 **0**、现在 **1**；该断言在 `89074fe` 上红、在工作树上绿 —— 两处声明（设计 §8.1 A5 ＋ 本档 A 批 A5 行）均已订正为「第三态由本轮补齐」 |
| D2 | 三态两条断言（`读窗为空 ⇒ 整段不打印` / `无时间戳 ⇒ 打「无法计算」句`）＋ 聚合口径一条（下 D10）在 1127 全绿里；设计 §4.2 与 README 的同段文字与之一一对应 |
| D3 | 状态卡/列表/team_read 的全部成本断言仍绿（`U7` ×3、`U11`、`U14` 超额标注）；设计 §4.1 的新表两个工具两种形态的读数与 `lib/index.js:639-641`（拒绝）与 `:1890`/`:2866`（截断标注）逐条对得上 |
| D4 | 四条断言全绿；`grep ptc-dispatch host-half.test.mjs` 从 **0** 命中 → 夹具里出现（此前只在实现与 README 里） |
| D5 | `readCall({team, readIds: "session-worker-b"})` ⇒ `【调用失败：…"readIds" must be an array】` 且该次 `surfaceReads` 计数**不变**（= 0 增量） |
| D6 | README 设计文档索引那行与父档 §7 第 161 行的状态措辞一致（步 2 / 4 / 6 已实施；步 3 / 5 未开工）——两处 grep 比对 |
| D7 | 修复前 `1127 (failed: 1)`（唯一红就是它）→ 落码后 `1127 (failed: 0)`；断言的另一半反向钉住「字面 id 的行**不**带该标注」 |
| D8 | 设计 §4.4（判据 ＋ 示例行 ＋ U14 行）与 README 四态表各出现第 4 态，且措辞只陈述 `outcome`；`U14 非 delivered…` 既有断言仍绿 |
| D9 | 该断言的恒真半边已换成真读数（两次调用各恰 12）；换真读数后仍绿 ⇒ 注记确实零额外读 |
| D10 | 聚合上限断言在 1127 全绿里（`M` = 5 且两个团队台账尾都在卡上）；设计 §4.2 成本列已回填为「单团队 1 次 / 聚合 N 次」 |
| D11 | 设计 §4.2 那一行已改为 `ctx.sessionQuery.listSessions`；模块级 `inject` 恒 4 项（既有断言）且其中**没有** `sessions` —— 与实现同源 |
| D12 | 非 banner 的本会话消息归到 `t-7` 且标「（本会话）」的断言全绿；设计 §4.3 写死了同一个归组面 |

### 计数与同批同步

- `README.md`：tests 徽章 `1117 → 1127`、「当前读数」`1117 → 1127`、Agent Teams 对照表的稳定性格 `1117 → 1127`；状态卡段补 D2 的三态与聚合口径；收件视图/派生回执段补 D12 归组面、D4 的两类载体 ＋ `callId` 配对 ＋ 窗口单位、D7 的「按当前名册解析」标注；回执表补 D8 的第 4 态；设计文档索引那行的状态与父档对齐（D6）；
- `CHANGELOG.md`：新增「分歧审计修复轮 DIVERGENCE(12)」条目（修了什么 → 为什么 → 怎么验证 → 同批同步的面），含本轮 10 条断言的逐条红相/负相交代；
- `host-half.test.mjs`：新增 10 条断言 ＋ 4 条夹具事件（非 banner 消息、`tool/ptc-dispatch`、回执截断、18 目标的一次投递）＋ 3 个新夹具（无时间戳 / 读窗为空 / 两团队聚合）＋ `trTruncatedResult` 助手；`U13 零额外读` 那条改成真计数（D9）；
