...
- **修复后**：`question` = 「确认创建 2 个 worker 会话并登记进团队 night-shift？」（**单行、≤120 码点**）；`detail` = 披露正文整段（**逐字等于 `teamSessionDialogText` 的产物**，两个预算都断言）。常规用例（`n=2 team=night-shift roles=worker-a,worker-b task=做接口`，协调者 `session-self`）实测 **578 / 600 码点、9 / 12 换行**；压力用例（8 会话 + 3000 字正文 / 39 字团队名 / 110 字 cwd / 两者都长）实测 **542 / 495 / 576 / 550 码点、均 9 换行**，全部在预算内，被裁的用例都带「已裁剪至 600 码点 / 12 行上限」标注。
