...
**本节的代价与残留（如实记）**：① 真机复验在用户 roster 里留下 **3 个一次性探针团队**（`rm-probe-20260922` · `rm-release-20260922` · `dsh-session-link-pro-20260922`）与其会话 —— 工具面删不掉团队行（只有设置 UI 能删），已向用户声明；② 面 2 的那次命令**按了取消**，所以「带日期默认名 ＋ 释放披露」这一格只有**框体读数**、没有落笔（落笔那一格由面 1 的显式 `team=` 命令覆盖）；③ 「渲染高度不超视口」这一条真机项本轮**已被动验到**（截图里动作行全在视野内），但它依赖窗口宽度，仍不作单测断言。
