...
## §10.2.8.9 / §10.2.8.10 批（2026-09-22；当次实测 `994 (failed: 0)` / `269 (failed: 0)`，基线 `971 (failed: 0)` / `269 (failed: 0)`）

**设计（唯一事实源）**：`docs/collab-enhancements-design-2026-09-19.md` **§10.2.8.9**（① 默认名带日期 · ② 现任失联 ⇒ 释放并认领）与 **§10.2.8.10**（① 调用方回执 = 通道 4 · ② 无任务也投最小唤醒），以及 §10.2.8.4 的 U33c 判据。**+23 条断言**（971 → 994）。
...
| # | 把哪一行改坏 | 读数 | 红的恰是哪条 |
| --- | --- | --- | --- |
| A | (乙) 的 `unknown` ⇒ 「读不到就释放」（两处 return） | `994 (failed: 1)` | **U37 (乙)**（getter 抛错 / 不是数组那一具） |
| B | 「只看现任」⇒ 看任一历史任（handler 取 `history[0].session`） | `994 (failed: 2)` | **U37 变异基线 2** ＋ 一条**既有计时器夹具的偶发红**（见下） |
| C | 写时复检去掉活性那一半（`revived = false`） | `994 (failed: 1)` | **U37 变异基线 3（TOCTOU）** |

三条变异各自跑完即**逐字还原**，还原后复跑 `994 (failed: 0)`。

### 隔离夹具的红相（新测试 ＋ 旧实现）—— **当次实测 `994 (failed: 33)`**
