...
## §10.2.8.9 / §10.2.8.10 批（2026-09-22；当次实测 `993 (failed: 0)` / `269 (failed: 0)`，基线 `971 (failed: 0)` / `269 (failed: 0)`）

**设计（唯一事实源）**：`docs/collab-enhancements-design-2026-09-19.md` **§10.2.8.9**（① 默认名带日期 · ② 现任失联 ⇒ 释放并认领）与 **§10.2.8.10**（① 调用方回执 = 通道 4 · ② 无任务也投最小唤醒），以及 §10.2.8.4 的 U33c 判据。**+22 条断言**（971 → 993）。
...
| # | 把哪一行改坏 | 读数 | 红的恰是哪条 |
| --- | --- | --- | --- |
| A | (乙) 的 `unknown` ⇒ 「读不到就释放」（两处 return） | `993 (failed: 1)` | **U37 (乙)**（getter 抛错 / 不是数组那一具） |
| B | 「只看现任」⇒ 看任一历史任（handler 取 `history[0].session`） | `993 (failed: 2)` | **U37 变异基线 2** ＋ 一条**既有计时器夹具的偶发红**（见下） |
| C | 写时复检去掉活性那一半（`revived = false`） | `993 (failed: 1)` | **U37 变异基线 3（TOCTOU）** |

三条变异各自跑完即**逐字还原**，还原后复跑 `993 (failed: 0)`。

### 隔离夹具的红相（新测试 ＋ 旧实现）

**旧实现** = 本批改动前的 `HEAD:lib/index.js`（含 §10.2.8 与裁定 A 的全部修法、不含本批），**新测试** = 本树 `host-half.test.mjs`：新夹具逐条红（U35 五条 / U36 八条 / U37 四条 / 通道 4 六条 / U30 反转三条 —— 逐条都要求新语义、旧实现下必然红）。**客户端半边一字未改**（`269 (failed: 0)`）。
...
1. **§10.2.8.10 的实现期兜底没被触发**：设计给的是「先试 handler 内 `followup`，任一条冒烟判据不过就改结算后定时器」。本批**先试的那条路**就成立 —— ① 恰一个回合（断言）② `command/done` 仍落盘（未触碰结算路径）④ 重试不双投（`followup` 只有一处调用点）。**③「不阻塞」与真机面（导出含 `turn/start`、会话不再 blank）仍是真机项**：本轮**未做真机验证**（宿主半边需 DSH 重启），**不得声称已验证**。
...
6. **`README.md` 的两处计数已同批更新**（徽章 `966 → 993`、「当前读数」`966 → 993`）—— 本仓「计数与列表同改」：本批**新增了 22 条断言**，因此这两处**必须**动（与上一批「没有变化 ⇒ 没有编辑」相对）。另在「十、测试」下加了一段**用法变化**（空会话里发命令会有回执把会话显出来 ＋ 代价）。
