...
| **DEFECT-1 / DEFECT-3（会话跑不起来）** | ❌ **未闭合**——重启后复验仍报 `{{model}} has no value (deployment:persona-prefix)`。真因是 **DEFECT-3**（缺省模型选择从未被解析），见 `.goal/DEFECT-3-model-selection-missing.md`；**修复轮进行中** |
