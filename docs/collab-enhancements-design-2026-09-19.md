...
- **框内披露（2026-09-22 用户裁定「要」；审计 #6 补强、R2 🟡#2/#3 定形）**：当 `/team_session` 触发释放并认领时，**必须在确认框的必备披露里写出来**，**形状定死**：**12 码点短句 `（将释放并接管原团队）`，折叠进既有的「确认则」那一行**（团队名已在框内第 1 行 ⇒ 不重复）。**实测代价**：仍 **6 行**；交付码点 **364**（12 码点 id）/ **380**（28 码点 id）≤ **390** ✓。**禁止**改用中/长句式（22 码点起即破 390 —— 实测 375/391、390/406）或**另起一行**（会破「≤6 行」）。与 §10.2.4「批量动作必须有一处人类确认且写明将建立的信任」同源。
