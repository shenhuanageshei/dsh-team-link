...
| **DEFECT-4（会话命名，2026-09-20 用户发现）** | ❌ **真缺口**：新建会话**全部叫工作区名**（`dsh-session-link-pro`）⇒ 侧边栏里**互相无法区分**。曾以「命名是用户可见的交互决定，设计没给就不自造」为由不设标题（`dsh-webhook` 则调 `sessionTitle.rename`）——但**默认值本身已经替用户做了一个很糟的决定**。修法：按已有结构化信息给出**可区分**的默认标题（`<team> · <role>`），并在回执里说明设了什么 |
...
| **无人值守 claim 的读数歧义（新，2026-09-20 真机发现）** | ⚠️ 本次 `claim` **无人值守**：确认框约 2 分钟超时，**调用方读到的是「超时」**，而**变更其实已经提交**（落在设计规定的「全部 provisional 迁移」分支）⇒ coordinator 的 14 条 pairs 现为 **provisional，2026-09-21 15:03:23 到期未批准则自动回退为过门投递**。**插件行为正确**（attended-only、无无人值守变体是设计如此），但**「对话框超时」与「调用失败」在调用方读数上同形、后果完全不同**（已换届 vs 未换届）——这是**可观测性缺口**，记为待修 |
