...
> - 现测：host **831** / client **138** 全绿。已发布部分的说明书在**主文档** `team-upgrade-design-2026-09-17.md` 的 §1–§9（文首有「文档地图与状态」表）。
...
> **状态（2026-09-20 更新）**：**③a 主路径**（`successor:"auto"` + 三层交接文档契约（5 硬节 + 缺项阶梯）+ `/team_rotate`）**与 ③b 恢复工具**（`team_link_recover`：恰两个封闭动词 + 活性诊断面 + 八条反后门约束）**均已实现**；两者各自的**差异审计与修复轮均已收口**。现测 host **831** / client **138** 全绿。**未发布**；**真机演练 10** 与 **H3** 待用户批准的重启窗口。**尚未闭合项**见 §11.9.9（`revive` 侧的身份复检仍是**半空锁**，需一条对称判据或收窄措辞）。
