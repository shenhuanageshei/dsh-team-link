...
[![tests](https://img.shields.io/badge/tests-1057%20%2B%20269%20assertions-brightgreen)](#十测试)
...
| 稳定性 | 实验原型，不承诺稳定性，schema 可自由变更 | 1057 + 269 条断言 + 变异验证（红相必红：本批新判据打在**修复前**的实现上**实测红 8 条**，读数见 [`docs/verification-log.md`](docs/verification-log.md)） |
...
| `team_link_team_read` | 一次读齐 roster + decisions 末 20 条 + discipline 全文 + tasks 末 20 条与派生视图 + 三个 baseHash |
...
**当前读数（实跑时点，一律取套件自报的那两行）**：`node host-half.test.mjs` → **1057**（failed: 0）；`node client-half.test.mjs` → **269**（failed: 0）。
...
| [`docs/team-ledger-and-mode-design-2026-09-26.md`](docs/team-ledger-and-mode-design-2026-09-26.md) | **「台账 + 形态」设计（2026-09-26 已拍板 · 第 1 步「只追加台账 `tasks.md`」已实施，判据 U1–U18；形态开关等第 2–5 步待实施）**：只追加台账 `tasks.md` + `meta.ref` 映射（把「账」搬出上下文、把「事」留在上下文）+ 团队形态开关（多会话 ↔ agent team 单会话兜底档）+ 只读团队状态卡 + 四条纪律约定；含 §6 拍板结论与 §7 实施顺序 |
