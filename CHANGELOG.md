...
- **修复前必红**（隔离夹具 `.tmp-head/`：本轮的新测试 + `git show HEAD:lib/{index,client}.js` + HEAD 的 `README.md` / `package.json`）：`node host-half.test.mjs` → `assertion total: 951 (failed: 30)`；`node client-half.test.mjs` → `assertion total: 269 (failed: 22)`。
  - 宿主侧 30 条里，本批新增的 **U30（9 条）/ U31（4 条）/ U33（9 条）全红**。其中 U33 那两条把**旧的槽位读数逐字留在案上**：修复前 `question` = **1068 码点 / 14 换行**（正是「长文放在 header」的形状），而 `detail` 只有 **63 码点的一行话** —— 换槽位把这两个数整个对调了。
...
- **原来那句「本命令是 §10.2.4 的批量动作：…」被**替换**，不是被删掉**：按修法表第 0 行，`detail` 现在整段承载披露正文；该句描述的批量范围由**必备披露块**「确认则：创建 → followup 投递启动任务 → 登记 roster 与 pairs；取消则零创建、零 pairs。」承接。**为什么两全不了（实测）**：正文预算在常规用例下已是 **578 / 600 码点**（另有 9 / 12 换行），任何**必备**块的增加都会挤掉可选的自述块 —— 实测会先掉 DEFECT-4 的标题段，进而让「确认框说明设了什么标题」那条既有断言变红。
...
- **`README.md` 的三处读数在本批之后仍未同步**（README 不在本批的文件清单内）：第 7 行的 tests 徽章 `927 + 259`、第 863 行「当前读数」`927` / `259`、第 880 行设计档索引里的「**判据 U30–U34，未落码**」——应分别改为 `951 + 269` / `951` / `269` /「已落码（2026-09-22，见本题）」。**本批未改 README**，如实记为待父侧同步项。
