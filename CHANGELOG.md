...
- **原来那句「本命令是 §10.2.4 的批量动作：…」被**替换**，不是被删掉**：按修法表第 0 行，`detail` 现在整段承载披露正文；该句描述的批量范围由**必备披露块**「确认则：创建 → followup 投递启动任务 → 登记 roster 与 pairs；取消则零创建、零 pairs。」承接。**为什么两全不了（实测）**：正文预算在常规用例下已是 **594 / 600 码点**（另有 9 / 12 换行），任何**必备**块的增加都会挤掉可选的自述块 —— 实测会先掉 DEFECT-4 的标题段，进而让「确认框说明设了什么标题」那条既有断言变红。
