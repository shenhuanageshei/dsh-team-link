...
**③ 怎么验证**（两套件实测 `993 (failed: 0)` / `269 (failed: 0)`，基线 `971 (failed: 0)` / `269 (failed: 0)`；**+22 条断言**）

- **变异验证三条（逐条实跑，每条只红它该红的那条）**：**(A) 把 (乙) 的 `unknown` 改成「读不到就释放」** ⇒ `993 (failed: 1)`，恰好红「U37 (乙)」；**(B) 把「只看现任」改成「看任一历史任」** ⇒ 恰好红「U37 变异基线 2」（同跑另见一条**既有计时器夹具的偶发红**，见下）；**(C) 去掉写时复检里的活性那一半** ⇒ `993 (failed: 1)`，恰好红「U37 变异基线 3（TOCTOU）」。
- **红相（新测试 ＋ 旧实现，隔离夹具实跑）**：`993 (failed: 32)` —— 32 条逐条点名（U16 成本口径 1 · U33 2 · U33c 2 · U30 默认值 2 · U35 5 · U30 反转 4 · U32 通道 4 5 · U36 7 · U37 4），其中 3 条**负相**（`policy.writer` 边界 / 无新日志事件 / 只看现任）在旧实现上**本来就绿**，如实标注不算红相。**这次实跑还抓出我自己新测试的两个崩相**（未守卫的可测面读取 · 只在假分支求值的 `probe` 声明在后面 ⇒ `TypeError` / TDZ，`assertion total` 都不打印）—— 已按本仓 Y7 纪律修成「先取值再判类型」，教训记在 `docs/verification-log.md`。
...
- **如实标注**：**未做真机验证**（宿主半边需 DSH 重启；§10.2.8.10 的「不阻塞」与真机面「导出含 `turn/start`、会话不再 blank」仍是真机项）；**§10.2.8.10 的实现期兜底没被触发**（回执走的正是先试的 handler 内 `followup`）；**设计档的两处计数/读数勘误**已同批改（该披露句字面量逐字数 **11**，原写「12 码点」；§10.2.8.9 ② 的 `364 / 380` 是**改写成本行之前**的读数，重测后为 `348 / 382`）；**一次既有计时器夹具的偶发红**（「缺口1 超时读数自报真实后果（计时器到点）」，连续两次复跑均绿，与本批无关）如实记录、不计入本批证据。
