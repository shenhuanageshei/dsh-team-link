...
**③ 怎么验证**（两套件实测 `994 (failed: 0)` / `269 (failed: 0)`，基线 `971 (failed: 0)` / `269 (failed: 0)`；**+23 条断言**）

- **变异验证三条（逐条实跑，每条只红它该红的那条）**：**(A) 把 (乙) 的 `unknown` 改成「读不到就释放」** ⇒ `994 (failed: 1)`，恰好红「U37 (乙)」；**(B) 把「只看现任」改成「看任一历史任」** ⇒ 恰好红「U37 变异基线 2」（同跑另见一条**既有计时器夹具的偶发红**，见下）；**(C) 去掉写时复检里的活性那一半** ⇒ `994 (failed: 1)`，恰好红「U37 变异基线 3（TOCTOU）」。
- **红相（新测试 ＋ 旧实现，隔离夹具实跑）**：`994 (failed: 33)` —— 33 条逐条点名（U16 成本口径 1 · U33 2 · U33c 2 · U30 默认值 2 · U35 5 · U30 反转 4 · U32 通道 4 5 · U36 8 · U37 4），其中 3 条**负相**（`policy.writer` 边界 / 无新日志事件 / 只看现任）在旧实现上**本来就绿**，如实标注不算红相。**这次实跑还抓出我自己新测试的两个崩相**（未守卫的可测面读取 · 只在假分支求值的 `probe` 声明在后面 ⇒ `TypeError` / TDZ，`assertion total` 都不打印）—— 已按本仓 Y7 纪律修成「先取值再判类型」，教训记在 `docs/verification-log.md`。
