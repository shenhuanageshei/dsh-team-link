...
1. **§4.3 入口**：侧栏底部新增「会话工具」——注册进官方 list 槽位 `sidebar.footer.action`（`id: team-link-session-tools`、`order: 0` ⇒ 落在消耗卡片 `order: -10` 之下、【设置】之上），**不改任何官方文件**。宽态 = 图标 + 文字「会话工具」，收起态（56px 轨道）**只渲染图标**（文字转 `aria-label`/`title`）；点击 / Enter / Space 打开官方 **`Modal`**（居中、挂 body——锚定面板会被 56px 轨道裁切）。
