...
> **发布状态**：①② 属宿主半边，需 DSH 重启才在真机生效；③④ 属浏览器半边（客户端 bundle 重建 + 刷新页面）。真机复验点 1 的**拒绝面已闭合**（2026-09-21 重启后复跑，原始读数见 `docs/verification-log.md`「批次 4 · H 真机复验点 1 · 绿相（重启后）」一节）；该点的「**同源带 cookie → 200 全量**」正路径与点 3 仍待复验 —— 与 0.3.8 同口径，**不把未验的东西说成已验**。
