...
## 0.3.10 — 未发布（§10.2.8 `/team_session` 三条真机缺陷修订：输入文法 · 结果可见性 · 确认框边界）
...
> **发布状态**：① ② 是宿主半边（需 DSH 重启才在真机生效）；③ 含**一号宿主半边**（换槽位）与**二号浏览器半边**（§10.2.8.3 通道 1 的客户端节点，需重建客户端 bundle + 刷新页面）。**U33 的「渲染高度不超视口」是设计显式降级的真机项**（依赖窗口宽度，刻意不作单测断言）；本条目**不声称已真机验证**。
