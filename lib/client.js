...
,
		...
		// kind, root scope, owner props exactly `{wide}`). `order: 0` puts this
		// entry BELOW the usage card (`order: -10`) and ABOVE 【设置】 — list entries
		// render in ascending `order` and the shell renders this slot before the
		// settings seat.
		//
		...
		/** The entry's glyph: the two sizes 【设置】's row uses (16 in the wide column,
		 * 18 in the rail). A seed module that ships without these icons falls back to
		 * a text glyph — a missing icon must not cost the row. */
