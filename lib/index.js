 ...
 *
 * DEFECT-3 收尾 (裁定 2): the model slot says the same. It used to read
 * 「（本会话默认）」 whenever `model=` was absent — the wording of a judgement that the
 * real machine has since refuted (an agent created without `agentOptions` never reaches
 * the host's own default), and it is also false for the half-route caller. Every shape
 * now names what will really be written ({@link teamSessionModelLine}).
 */
function teamSessionDialogText(plan, cwd, coordinatorId) {
	const taskLines = plan.sessions.map((entry) => `  - ${entry.role} → ${entry.sessionId}${entry.skip ? "（已登记，跳过）" : `（标题：${teamSessionTitle(plan, entry)}）`}`);
	// The DEFECT-4 sentence names the rule AND the failure mode it replaces, because
	// 「每个新会话都会被设一个标题」 alone would not tell the user why it matters
	// (they have just seen a sidebar full of identically-named sessions).
	const firstRole = plan.sessions.find((entry) => !entry.skip)?.role;
	const titleSample = firstRole === undefined ? undefined : teamSessionTitle(plan, { role: firstRole });
	return wellFormed([
		...
		...taskLines,
		`- 会话标题（DEFECT-4）：每个新会话按「<team> · <role>」命名${titleSample === undefined ? "" : `（如 ${titleSample}）`}——不设标题时它们会全部显示为工作区名，在侧边栏里互相无法区分；团队名过长时会截**团队段**（保留前缀 + 省略号），**role 段始终完整**（宿主的标题上限按字节剪尾巴，剪掉的若是 role，同队两个角色就会撞成同一个名字，缺口2）；想改随时在壳里重命名。`,
		`- 模型/预设：${teamSessionModelLine(plan)} · preset=${plan.preset === undefined ? "（宿主缺省 agentPreset——缺省也解析并挂载，§10.2.2）" : plan.preset}`,
		`- 工作目录（cwd）：${cwd}（绝对路径）`,
...
function teamSessionModelLine(plan) {
	if (plan.provider !== undefined && plan.model !== undefined) return `model=${plan.model}（provider=${plan.provider}）`;
	const resolved = "解析并带上宿主缺省模型选择（agentDefaultModel.currentSelection()，§10.2.2 模板第 ④ 步）";
	if (plan.provider === undefined && plan.model === undefined) return `未给 model= / provider ⇒ 两半都${resolved}`;
	return plan.model === undefined
		? `provider=${plan.provider}（缺的 model 那一半）⇒ ${resolved}`
		: `model=${plan.model}（缺的 provider 那一半）⇒ ${resolved}`;
}
...
async function reapIncumbent(deps, request) {
	...
	const incumbent = preflight.incumbent;
	// §4.2 (c) ①: the capability gate runs HERE — before the box, not after the pick.
	// §4.2 (c) states it as a precondition of the whole verb (the same placement the
	// auto path uses at its own entry), and the acceptance criterion is explicit: a
	// host without `agents.create` must get a fail-closed REPORT and **zero dialogs**,
	// rather than a box whose「自建继任者」option could never be honoured.
	if (typeof controller.teamSession?.rootCtx?.agents?.create !== "function") {
		return { error: `恢复失败（reappoint，${SELF_SUCCESSOR_LABEL}）：本宿主没有可用的 agents.create（编程创建会话不可用）——插件无法新建继任者。本次零创建、零令牌、零 freeze、零写入（连确认框都不弹）。可走的路径：① 先在壳里新建一个会话，再用 team_link_rotate action=prepare 把它的 id 作为 successor 传入（显式 successor 不需要 agents.create，手工路径始终保留，§3.6.4）；② team_link_recover action=revive（仅插件自建会话）；③ 由用户在设置 UI 直接改现任。${RECOVERY_NO_WRITE_SUFFIX}` };
	}
	// §4.2 (c): the candidate set is 活成员 ∪ 插件自建继任者（常驻）. The synthetic
	// entry is ALWAYS here, so the "no live member to hand the role to" refusal that
	// used to live at this line is now unreachable and is gone with it — candidates
	// running out was exactly the state that had no exit, and 「排到没有候选」 is not
	// a state the dialog may still pretend cannot happen.
	...
	const optionText = candidates.map((candidate) => candidate.synthetic === true
	...
	const dialog = await askRecoveryCandidates(ctx, agent, {
		...
		question: [
			...
			`- 若选「${SELF_SUCCESSOR_LABEL}」：代价如实声明——继任者是**空上下文的新会话**（前任的历史/对话**不迁移**），它拿到的上下文只有插件从 roster 事实生成的交接文档；信任靠它本人凭令牌 claim 时逐项勾选迁移，或无人值守时全部 provisional + 24h 回退。`,
	...
	// the published mechanism, verbatim: capability gate → §10.2.2 id → hand-over
	// document → prepare → the same three audit trails.
	if (picked.synthetic === true) {
		return await appointSelfBuiltSuccessor({ ctx, policy, rotation, controller }, { teamName, roleName, caller, agent, signal, now, incumbent, team: teamNow, entry: entryNow, candidates });
	}
	...
	return {
		lines: [
			...
			`- 候选集（宿主计算，模型不可指定）：${candidates.map((candidate) => candidate.synthetic === true ? SELF_SUCCESSOR_LABEL : `${candidate.role}→${candidate.session}`).join("、")}`,
			`- 审计留痕（§11.9.5⑦，三处）：① 版本史备注 ${row.note}；② ${mirrorNote(trails.mirror)}；③ 黑板 ${trails.decisions}`,
			...prepared.lines,
...
function selfBuiltHandoffBody(request) {
	...
	const lines = [
		"## mission",
		`接管团队 ${request.teamName} 的角色 ${request.roleName}（本会话由 team_link_recover action=reappoint 的「${SELF_SUCCESSOR_LABEL}」路径新建：该角色的前任在本进程没有活动代理，而全队没有可改任的活成员）。`,
		...
		"",
		"## task-and-goal",
		"- 该角色的任务与目标：**未知**——前任没有活动代理，goals 按 agent 取读数，因此取不到；本文件禁止编造。",
...
/**
 * §4.2 (c): the whole chain for the synthetic successor, in the design's order
 * and with nothing re-spelled — capability gate → §10.2.2 id → the SAME creation
 * path `/team_session` and `successor:"auto"` use → the hand-over document →
 * the published `prepare` VERBATIM → the same three audit trails.
 *
 * The order of the last three follows §11.9.6 (the document is written BEFORE
 ...
 * the design's numbering names the ELEMENTS, and §11.9.6 is the ordering.
 *
...
async function appointSelfBuiltSuccessor(deps, request) {
	...
	return {
		lines: [
			...
			`- 交接文档（§11.9.6，插件从 roster 事实生成的五硬节）：${document.path}（上一份：${document.previousDocument ?? "（无——本角色落盘的第一份）"}）——读不到的项（前任的进行中工作、它的 goal、未提交的改动）在文档里**如实标未知**，没有编造任何内容。`,
			titleLine,
...
function recoveryDiagnosticLines(team, now, isLive) {
	...
	lines.push(`- 恢复入口：team_link_recover action=revive（复活当前 current 这个会话本身，仅插件自建会话，**任意角色**）或 action=reappoint（改任给本队活成员；没有活成员时可由插件**自建继任者**，走完整 M4 交接）。`);
...
function registerRecoveryTools(ctx, policy, rotation, controller, teamSession) {
	ctx.effect(() => ctx.tools.register(defineTool({
		...
		description: [
			...
			"action=reappoint：现任不会/不应再回来时的「人改任」= 人类对话授权的 prepare。候选 = **本队活成员 ∪ 常驻的「自建继任者（新建会话）」**（§4.2 (c)），由**插件**算出并作为对话框选项呈现（模型只传 team，可选 role，**不能指定任何继任者 id**——合成候选连 id 都还没有，它由插件在人类勾选之后才铸）；人类勾选后逐字走既有 M4：铸一次性令牌绑定 (team, role, successor)、把待迁移信任快照进 rotationBackup、广播 rotation-freeze；随后由继任者本人凭令牌 claim（claim 逐字不动，不新增令牌类型）。选「自建继任者」时的链路同样逐条复用既有机制：能力闸门（宿主无 agents.create ⇒ fail-closed 报告、本次零创建零令牌零 freeze）→ 按 §10.2.2 铸 id（team-link-<team>-<role>-<uuid8>）→ 交接文档由**插件从 roster 事实**生成五硬节（读不到的项——前任的进行中工作与 goal、未提交的改动——**如实标未知**，不编造）→ prepare 逐字跑 → 审计行 verb=reappoint。代价如实声明：继任者是**空上下文的新会话**，历史不迁移；信任靠它 claim 时逐项勾选迁移。",
