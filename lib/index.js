 ...
 * 收紧后的正文（六行 / 参照夹具 345 码点）离它们更远，而**可裁段为空** ⇒ `dialogFits` 目前在
 * 常规路径上不再被调用（见 {@link teamSessionDialogText} 的「可裁段现在是空的」一段）。 */
 ...
 *   ⑤ 成本口径 ＋ 信任授予 · ⑥ 确认则。
 * 参照夹具（`/team_session 你是新的主管会话` ＋ 12 码点协调者 id）实测 **345 码点 / 6 行** ≤ 目标
 * **≤6 行 / ≤350 码点** ✓（同一夹具取满 28 码点协调者 id 时 361 —— 判据按 U33c 的夹具定义取 12）。
 *
 ...
 * 长 cwd ＋ 取满 28 码点的协调者 id）在**收紧后的这版正文**里是 **607 码点 > 600**（收紧前 693），
...
function teamSessionDialogText(plan, cwd, coordinatorId) {
	...
	const costLine = plan.task === undefined
		...
		: `成本：${plan.creating.length} 个会话 × 至少一个完整回合（followup 驱动一次）`;
	...
	const blocks = [
		...
		// ≤350 码点**（改前 569 码点 / 10 行）—— 由 host-half 的 U33c 双相断言咬住（把删掉的
		// 解释性文字加回一处 ⇒ 码点侧变红）。
