 ...
 * `[a-z0-9-]+` with no length bound, a cwd is a path, a session id is opaque. They are
 * WHY the required blocks fit the budget for every input — without them
 * 「不得压掉必备披露」 and a hard budget could not both hold. */
const TEAM_SESSION_DIALOG_FIELD_CHARS = { team: 20, cwd: 24, id: 28, role: 8, skipped: 16, title: 40 };
 ...
 * {@link TEAM_SESSION_DIALOG_MAX_NEWLINES}) plus three bounded renderings:
 *   a. the task preview is annotated ({@link teamSessionTaskNote}) — 「共 N 字，
 *      此处仅显示前 M 字；完整正文会原样作为启动任务投递」;
 *   b. the session list is capped at {@link TEAM_SESSION_DIALOG_LIST_ROWS} rows
 *      plus 「…等 N 个」, with the FULL count always stated (the U13/U14 bounded-
 *      presentation convention);
 *   c. DEFECT-4's title paragraph and the model/preset self-description are ONE
 *      line each.
 *
 ...
 * them is capped ({@link TEAM_SESSION_DIALOG_FIELD_CHARS}), which is what makes the
 * required set fit the budget for EVERY input.
 *
...
function teamSessionDialogText(plan, cwd, coordinatorId) {
	...
	// DEFECT-4's readings: the SAME derivation `renameTeamSession` hands to the title
	// service, capped like every other list ({@link TEAM_SESSION_DIALOG_LIST_ROWS}).
	const titleRows = plan.sessions.slice(0, TEAM_SESSION_DIALOG_LIST_ROWS).map((entry) => `${dialogField(entry.role, cap.role)}（标题：${dialogField(teamSessionTitle(plan, entry), cap.title)}）`).join("；");
	...
	const blocks = [
		// ---- §10.2.4 的必备披露 —— 永不裁剪 --------------------------------------
		{ required: true, text: `将创建 ${plan.creating.length} 个 worker 根会话并登记进团队 ${dialogField(plan.team, cap.team)}${skippedNote}。` },
		{ required: true, text: `- 会话（共 ${roles.length} 个${hidden > 0 ? `，仅列前 ${shown.length} 个` : ""}）：${shown.join("；")}${hidden > 0 ? `；…等 ${roles.length} 个` : ""}` },
		{ required: true, text: `- 模型/预设：${teamSessionModelLine(plan)} · preset=${plan.preset === undefined ? "宿主缺省" : dialogField(plan.preset, cap.role)}` },
		{ required: true, text: `- 工作目录（cwd）：${dialogField(cwd, cap.cwd)}` },
		{ required: true, text: `- 启动任务：${teamSessionTaskNote(plan.task)}` },
		// §10.2.8.7 裁定 2: one line, and it is inside the budget below.
		{ required: true, text: TEAM_SESSION_ROLE_GUIDANCE },
		{ required: true, text: `成本口径（保守）：${plan.creating.length} 个会话 × 至少一个完整回合（followup 驱动一次，按各自模型计费）。` },
		{ required: true, text: `信任授予：与主会话 ${dialogField(coordinatorId ?? "（当前会话）", cap.id)} 建立 pairs 配对——双向免确认通道（绕过发送方审批与接收方 ask 两道门，§10.2.3 预置配对）。` },
		{ required: true, text: `确认则：创建 → followup 投递启动任务 → 登记 roster 与 pairs；取消则零创建、零 pairs。` },
		// ---- 自述段：超预算时先压这一段（§10.2.8.4 (b)）-------------------------
		// DEFECT-4's self-description: ONE line of rule + the titles read off the SAME
		// derivation the create path hands to `rename` — cropped (with the annotation)
		// when a large batch leaves no room, which is what §10.2.8.4 (b) 的自述段压缩 means.
		{ required: false, text: `- 会话标题（DEFECT-4）：${titleRows}——不设标题时它们会全部显示为工作区名，在侧边栏里互相无法区分；想改随时在壳里重命名。` },
	];
	/** The blocks of one candidate body, joined the way the dialog renders them. */
	const priced = (list) => list.map((block) => block.text).join("\n");
	/** One candidate body INCLUDING the crop note it would carry: the note is priced
	 * at the count that would really remain after this decision, which is why an
	 * accepted last optional block still fits (nothing was dropped, so no note). */
	const candidate = (list, stillDropped) => stillDropped <= 0 ? priced(list) : `${priced(list)}\n${teamSessionDialogCropNote(stillDropped)}`;
	...
	let text = candidate(kept, dropped);
...
/** The annotation every crop carries (§10.2.8.4 (b): 超出即按段落裁剪并标注). It names
 * what was kept, so a reader of a cropped box knows the disclosure is intact. */
function teamSessionDialogCropNote(dropped) {
	return `（已裁剪至 ${TEAM_SESSION_DIALOG_MAX_CODE_POINTS} 码点 / ${TEAM_SESSION_DIALOG_MAX_NEWLINES} 行上限，省略 ${dropped} 段自述；必备披露一字未少。）`;
}
...
function teamSessionTaskNote(task) {
	if (typeof task !== "string" || task.trim() === "") return "（未给正文/task=，只投递角色与汇报方式）";
	...
	return total <= TEAM_SESSION_DIALOG_TASK_CHARS
		? `共 ${total} 字，完整正文会原样作为启动任务投递：${shown}`
 ...
 *
 * §10.2.8.4 (b) 压到一行：the locked phrases (`两半都解析并带上宿主缺省模型选择` /
 * `缺的 provider 那一半`) stay verbatim — they are the content the 裁定 2 judgement is
 * ABOUT — and the parenthetical around them is what got shorter.
 */
