...
function teamSessionKickoffText(plan, role, cwd) {
	...
	if (typeof plan.task !== "string" || plan.task.trim() === "") {
		return wellFormed([
			...
			"- 请等待主会话派活：主会话会用 team_link_send 把任务发给你（本命令已为你们建立 pairs 双向免确认通道，投递不再弹确认框）。",
			`- 工作目录（cwd）：${cwd}`,
		].join("\n"));
	}
...
function teamSessionReceiptText(receipt) {
	...
	if (receipt.state === "success") {
		...
		return receipt.team === undefined ? `${label} 完成：${counts}` : `${label} 完成：团队 ${receipt.team}，${counts}`;
	}
			...
			handler: async (invocation) => {
				...
				try {
					...
					if (plan.creating.length === 0) {
						...
						return { kind: "success", receipt: { state: "success", team: plan.team, created: 0, minimalWake: 0 }, text: `${PLUGIN_LABEL} /${TEAM_SESSION_COMMAND}：团队 ${plan.team} 已登记本次请求的全部 ${plan.skipped.length} 个角色（按 role 幂等，§10.2.6），无需创建。零创建、零 pairs。` };
					}
					...
					return {
						...
						receipt: { state: failed ? "error" : "success", team: plan.team, created: createdRoles.length, minimalWake: outcome.results.filter((entry) => entry.minimalWake === true).length, reason: failed ? "批量建队未全部成功" : undefined },
