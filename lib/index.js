...
function rotationAutoDialogText(state) {
	return [
		...
		state.title === undefined
			...
			: `- 会话标题（DEFECT-4）：「${state.title}」——按「<team> · <role>」派生（不设标题会让它显示为工作区名，在侧边栏里认不出来）；想改随时在壳里重命名。`,
 ...
 *
 * **边界（如实声明，未自造第二套规则）**：标题的最终长度由**宿主的标题服务**决定
 * （`dsh-base` 配置里 `maxTitleBytes: 80`，`dsh-session-title` 按字节**优雅截断**、
 * 不抛错），而团队名只受 `[a-z0-9-]+` 约束、**没有长度上限**（角色名上限 64 字符）
 * ⇒ 一个极长的团队名（70 字符以上）能把角色段截掉，让同队两个 role 撞成同一个前缀。
 * 这里取的是设计逐字规定的 `<team> · <role>` 形状，**不**自造「截团队、保角色」的第二套
 * 规则；真实团队名都很短，这一档交给宿主的标题上限兜（本函数的返回值已经被截断过的话，
 * `rename` 记录到的就是宿主接受的那一份）。
 */
function teamSessionTitle(plan, entry) {
	const team = typeof plan?.team === "string" ? plan.team.trim() : "";
	const role = typeof entry?.role === "string" ? entry.role.trim() : "";
	if (team !== "" && role !== "") return `${team}${TEAM_SESSION_TITLE_SEPARATOR}${role}`;
	if (team !== "") return team;
	const sessionId = typeof entry?.sessionId === "string" ? entry.sessionId : "";
	if (sessionId !== "") return sessionId.slice(0, TEAM_SESSION_TITLE_STEM_CHARS);
	return undefined;
}
...
function teamSessionDialogText(plan, cwd, coordinatorId) {
	...
	return wellFormed([
		...
		...taskLines,
		`- 会话标题（DEFECT-4）：每个新会话按「<team> · <role>」命名${titleSample === undefined ? "" : `（如 ${titleSample}）`}——不设标题时它们会全部显示为工作区名，在侧边栏里互相无法区分；想改随时在壳里重命名。`,
...
async function reapIncumbent(deps, request) {
	...
	if (candidates.length === 0) {
		return { error: `恢复失败（reappoint）：团队 ${teamName} 当前**没有活着的其他角色成员**可以改任（域 = 本团队存活成员，排除 ${roleName} 自己）。可能刚重载过插件、全队代理都拆了。可走的路径：① 在侧边栏逐个打开团队会话把它们拉回来（打开死亡现任本身就是 L1 的零成本版本）；② team_link_recover action=revive（仅插件自建会话）；③ 由用户在设置 UI 直接改现任。${RECOVERY_NO_WRITE_SUFFIX}` };
	}
	...
	if (!isLive(picked.session)) return { refused: recoveryRefusal(`恢复已中止（reappoint，写前复检）：候选 ${picked.session}（角色 ${picked.role}）在确认期间已经没有活动代理了——写入它会原地再造一个死结。零令牌、零 freeze`) };
...
function registerRecoveryTools(ctx, policy, rotation, controller, teamSession) {
	ctx.effect(() => ctx.tools.register(defineTool({
		...
		description: [
			...
			"角色窄域（★ 两个动词的域**不同**，照实现写，别再被读成「整个工具只服务 coordinator」——这是差异审计 Y1 的那处分叉）：**`revive` 只受理 coordinator 角色**（§11.9.1 的硬死锁只有一格：writer=coordinator 且死的是 coordinator；别的格子有既有的活路：retire + set-role），**`reappoint` 受理任意角色**——授权从不源自 coordinator 身份（唯一来源是对话框里人类那一下点击，§11.9.5），而「死的是 worker、活协调者的 writerGate 又被 writer=any 之外的策略卡住」时，reappoint 正是那条人改任路径。不带 role 时只输出诊断（每角色一行：现任 + 活性 + 在飞令牌），零副作用。",
		...
		parameters: {
			...
			team: { type: "string", required: true, description: "团队名（[a-z0-9-]+，须已在 roster 中）" },
			role: { type: "string", description: "要恢复的角色名（约定角色名 coordinator；revive 只受理 coordinator，reappoint 受理任意角色——不带 role 时只输出诊断，零副作用）" },
		},
		output: textOutput(),
