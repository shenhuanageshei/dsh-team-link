 ...
 * either half). They are WHY the required blocks fit the budget for every input —
 * without them 「不得压掉必备披露」 and a hard budget could not both hold. */
const TEAM_SESSION_DIALOG_FIELD_CHARS = { team: 20, cwd: 24, id: 28, role: 8, skipped: 16, title: 40, model: 40, provider: 24 };
...
function teamSessionDialogText(plan, cwd, coordinatorId) {
	...
	const blocks = [
		...
		{ required: true, text: `- 会话 id：${TEAM_SESSION_ID_SHAPE}（共 ${roles.length} 个${hidden > 0 ? `，仅列前 ${shown.length} 个` : ""}）：${shown.join("；")}${hidden > 0 ? `；…等 ${roles.length} 个` : ""}` },
		...
		{ required: false, text: `- 会话标题：${titleRows}——不设标题时它们会全都显示为工作区名；想改随时在壳里重命名。` },
	];
...
function teamSessionModelLine(plan) {
	...
	return plan.model === undefined
		? `provider=${dialogField(plan.provider, cap.provider)}（缺的 model 那一半）⇒ ${resolved}`
		: `model=${dialogField(plan.model, cap.model)}（缺的 provider 那一半）⇒ ${resolved}`;
}
