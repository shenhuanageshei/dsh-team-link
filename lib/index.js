...
function registerTeamTools(ctx, policy, rotation) {
	...
	ctx.effect(() => ctx.tools.register(defineTool({
		...
		description: "读团队黑板（M2，§3.3.3）：读取前先跑一次换届过期清扫（过期的 pending / provisional 窗口不因没人注册看门狗而漏），再返回 roster 概要 + decisions.md 末 20 条 + discipline.md 全文 + tasks.md 末 20 条与任务派生读数（逐任务的「最后主张 / 未消解存疑」——**是读数，不是裁决**）+ 各文件 baseHash（decisions 的**仅供参考/审计**——decisions 只追加、不接受 baseHash 参数；discipline 的供整文件替换的乐观锁使用）。任何会话可读；写黑板没有权限门，但读要一次读齐省轮次。文件不存在按空处理并如实标注。黑板根目录 = <team.workspace>/team/<name>/（workspace 在团队首次创建时从会话 agentCwd 捕获）。",
		parameters: {
