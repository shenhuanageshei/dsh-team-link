 ...
 *
 * DEFECT-3 之后，这条 early-return 只是**防御**：缺省分支同样会解析出真实的一对
 * （见 {@link buildTeamSessionCreateOptions}），所以本模块造出来的 agent 永远带着一个
 ...
 * `resolveRequest` 在调用方没给 model 时也解析宿主的缺省模型选择，把它写进
 * `agentOptions` 并装成 `setup` 里的初始模型选择（{@link resolveDefaultModelSelection}）。
...
async function buildTeamSessionCreateOptions(ctx, plan, entry, cwd) {
	...
	// DEFECT-3 的缺省分支（模板 `resolveRequest` 的 if 分支，`dsh-webhook:30-36`）：
	// 调用方没给 model 时**也**解析宿主的缺省模型选择，并把**同一份** selection 一路带到
	// `setup`（模板第 9 步）。原来这里是「什么都不填」⇒ `agentOptions` 为空 ⇒ 整个
	// `agentOptions` 都不传 ⇒ 编程创建的 agent 没有任何模型选择 ⇒
	// `{{model}}`（`dsh-agent-loop` 的
	// `ctx.systemPrompt.variable("model", (context) => context.agent?.options.model)`）
	// 无值，首回合死在 `prompt variable "{{model}}" has no value`——真机缺陷 #3。
	const modelSelection = plan.provider === undefined && plan.model === undefined
		? resolveDefaultModelSelection(ctx, entry.sessionId)
		: { provider: plan.provider, model: plan.model };
	if (plan.provider === undefined && plan.model === undefined) {
		agentOptions.provider = modelSelection.provider;
		agentOptions.model = modelSelection.model;
	}
...
/**
 * DEFECT-3（真机缺陷 #3）: 模板 `dsh-webhook` 的 `resolveRequest` **缺省分支**
 * （`lib/index.js:30-36`）——调用方没给 model 时，缺省选择也**必须**解析出来：
 *
 * ```js
 ...
 * 或拿到的 selection 不成对时**拒绝创建**（fail-visible，二选一里更硬的那个）：没有它就
 * 没有可解析的缺省模型，造出来的 agent 就是跑不起来的——「宁可不建，也不建一个跑不起来
 * 的会话」。调用方显式给了 provider/model 时不走这里。
 *
 * @throws 服务缺席，或 `currentSelection()` 给不出非空 provider/model 时。
 */
function resolveDefaultModelSelection(ctx, sessionId) {
	const agentDefaultModel = ctx.get?.("agentDefaultModel");
	if (agentDefaultModel === undefined || agentDefaultModel === null || typeof agentDefaultModel.currentSelection !== "function") {
		throw new Error(`agentDefaultModel service unavailable — 缺省模型选择解析不出来（真机缺陷 #3）：没有它就没有可用的 provider/model，新会话 ${sessionId} 的 agentOptions 只能是空的，而 deployment:persona-prefix 里 {{model}} 的值正是它 ⇒ 首回合会以 prompt variable "{{model}}" has no value 失败。拒绝创建：宁可不建，也不建一个跑不起来的会话。两条出路——传 model=<provider>/<model>（或让宿主的 agentDefaultModel service 可用）。`);
	}
	const selected = agentDefaultModel.currentSelection();
	if (typeof selected?.provider !== "string" || selected.provider === "" || typeof selected?.model !== "string" || selected.model === "") {
		throw new Error(`agentDefaultModel.currentSelection() 没有给出可用的 provider/model（provider=${JSON.stringify(selected?.provider)}, model=${JSON.stringify(selected?.model)}）——写进 agentOptions 的就是这一对，空值会原样复现真机缺陷 #3（新会话首回合报 prompt variable "{{model}}" has no value）。拒绝创建：宁可不建，也不建一个跑不起来的会话。两条出路——传 model=<provider>/<model>，或修好宿主的缺省模型设置。`);
	}
	return { ...selected };
}
