 ...
 * required blocks fit the budget for every input — without them 「不得压掉必备披露」 and a
 * hard budget could not both hold. */
...
function teamSessionDialogText(plan, cwd, coordinatorId) {
	...
	const blocks = [
		...
		{ required: true, text: `- 模型/预设：${teamSessionModelLine(plan)} · preset=${plan.preset === undefined ? "宿主缺省" : dialogField(plan.preset, cap.preset)}` },
		...
		// §10.2.8.2 默认值第 4 条下的**同支纪律**（2026-09-22 第 2 轮评审 🟡#1）：成本行与下一行
		...
		{ required: true, text: plan.task === undefined
			? `成本口径（保守）：本次 0 次驱动（未给正文/task=，不投启动任务；后续派活时按各自模型计费）。`
			: `成本口径（保守）：${plan.creating.length} 个会话 × 至少一个完整回合（followup 驱动一次，按各自模型计费）。` },
