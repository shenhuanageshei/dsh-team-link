...
function registerTools(ctx, policy, watchdog) {
	...
	ctx.effect(() => ctx.tools.register(defineTool({
		...
		description: "只读**团队状态卡**（2026-09-26「台账 + 形态」设计 §4.2）：一次调用一屏讲清团队现状，六段 —— ① 团队与角色（在位 / 空缺 / 版本史末条）② 换届 pending（在飞令牌：角色 / 继任者 / 到期时刻，**token 掩码**）③ 看门狗（谁盯谁 / 阈值 / 到期）④ 会话面（id / 代理状态 / 创建时间 / provisional 标记；零日志成本）⑤ 活性（**有界**：与 team_link_list_sessions 同一顺序读前 12 行，超出该窗口的行如实标「未读」，不用空数据算 verdict）⑥ 台账尾（tasks.md 末尾 tasksTail 条，默认 5、上限 20，超出拒绝）。**全只读**：调用前后 settings 与磁盘文件逐字节不变（不写、不缓存、不新增持久状态）；也不跑换届过期清扫（那会写）。team 省略 ⇒ 聚合本工作区全部团队（每团队一段）；给了但不存在 ⇒ 拒绝并列出已知团队名。调用方无会话身份时照常出卡，「当前会话」处如实标注（（当前会话未知）），**不编造身份**。末尾附 §4.2 的**反面预警注记**（窗口内 0 条消息 / 台账同期新增 M 行 ⇒ 会话可能已停止说话只写行——**读数，不是裁决**；零额外读：两个计数都取自本卡已读到的 surface 事件与台账行）。",
		parameters: {
