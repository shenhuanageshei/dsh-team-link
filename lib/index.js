...
//     §11.2 `successor:"auto"` hand-over creates through the same function, so
//     the fix covers the trust-migration path as well.
...
async function autoHandover(deps, request) {
	...
	let handle;
	try {
		handle = await createRootAgent(controller.rootCtx, await buildTeamSessionCreateOptions(ctx, { preset: undefined, provider: undefined, model: undefined }, { sessionId, role: roleName }, cwd));
	} catch (error) {
...
async function createTeamSessions(ctx, controller, policy, plan, cwd, coordinatorId) {
	...
	for (const entry of plan.sessions) {
		...
		let handle;
		try {
			handle = await createRootAgent(controller.rootCtx, await buildTeamSessionCreateOptions(ctx, plan, entry, cwd));
		} catch (error) {
