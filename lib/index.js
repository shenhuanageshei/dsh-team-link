...
function rotationAutoDialogText(state) {
	return [
		...
		`- 新会话 id：${state.sessionId}（插件自建、插件持有它的 AgentHandle；meta 只放 cwd，不含任何血统字段，§11.4.2 / §10.2.5）`,
		`- cwd：${state.cwd}`,
		`- 模型 / 预设：本路径不指定 provider/model/preset（新会话继承默认选择；§11.2 的入口没有这两个参数）`,
...
function handoffDeliveryMessage(state) {
	return [
		...
		"",
		"—— 以下为旧任提供的交接正文（五个硬节：mission / in-flight / commitments / unknowns / task-and-goal；插件只把关结构——节在场 / 非空 / 有界——不把关内容质量，§11.9.6）——",
		"",
		state.body,
	].join("\n");
}
...
async function autoHandover(deps, request) {
	...
	const view = policy.get();
	const admission = prepareAdmission(view, { teamName, roleName, successor: undefined, now, caller });
	...
	let handle;
	try {
		handle = await createRootAgent(controller.rootCtx, buildTeamSessionCreateOptions(ctx, { preset: undefined, provider: undefined, model: undefined }, { sessionId, role: roleName }, cwd));
	} catch (error) {
...
function registerRotationTools(ctx, policy, rotation, controller) {
	ctx.effect(() => ctx.tools.register(defineTool({
		...
		parameters: {
			...
			handoff: { type: "string", description: "prepare 可选：交接正文（Markdown）。五个硬节各以一个标题开头：mission / in-flight / commitments / unknowns / task-and-goal（软节 first-actions / team-map / conventions 缺了只警告）。successor:\"auto\" 时必须给（空正文或硬节缺/空 → 拒绝于工具入口，零建会话零令牌零 freeze）；显式 successor 时可选，给了就写入交接文档，缺了只警告。插件只把关结构（节在场 / 非空 / 有界），不把关内容质量（§11.9.6）。" },
		},
		output: textOutput(),
		...
		async execute(args, exec) {
			try {
				...
				if (action === "prepare") {
					...
					const notes = [];
					if (handoff.body !== "") {
						const admission = prepareAdmission(policy.get(), { teamName: name.value, roleName: role.value, successor: successor.value, now, caller });
						if (admission.error !== undefined) return wellFormed(admission.error);
...
async function createTeamSessions(ctx, controller, policy, plan, cwd, coordinatorId) {
	...
	for (const entry of plan.sessions) {
		...
		let handle;
		try {
			handle = await createRootAgent(controller.rootCtx, buildTeamSessionCreateOptions(ctx, plan, entry, cwd));
		} catch (error) {
...
/**
 * The ONE `agents.create` call site of this module (§10.2.2's template), shared
 * by the `/team_session` batch and the §11.2 `successor:"auto"` hand-over. Two
 * call sites would be two places to keep the root-session contract in (meta
 * carries cwd/agentPreset and nothing else, creation from the plugin root ctx),
 * so there is exactly one — and `host-half.test.mjs` counts it in the source.
 */
async function createRootAgent(rootCtx, options) {
	return await rootCtx.agents.create(options);
}
 ...
 * live in the body instead. ONE builder for every relay this plugin drives (the
 * §10.2.3 kickoff and the §11.4.5 hand-over delivery): the triple is a red line,
 * and a second copy of it would be a second thing to keep audited.
 *
...
const TEAM_ROTATE_KEY_HINT = "team=<name>（可选）";
...
function prepareAdmission(view, request) {
	const { teamName, roleName, successor, now, caller } = request;
 ...
 * refusal prints, of {@link HANDOFF_SECTION_HINTS} and of the header's integrity
 * line, so a section can never be enforced in one place and advertised in
 * another (the ② round's "same-fact list, half of it updated" lesson). */
...
function readHandoffArgument(value, options = {}) {
	...
	if (!present) {
		if (!auto) {
			return {
				body: "",
				report: null,
				warnings: [`未提供交接正文（handoff）：显式指定的继任者有自己的会话与上下文，M4 现有语义不因此收紧（§11.9.6 缺项阶梯）——但交接正文是头部/事实段之外唯一能留下判断的东西，建议下次仍按五硬节写一份：${HANDOFF_HARD_SECTIONS.join(" / ")}。`],
			};
		}
		return {
			error: `prepare 被拒绝（successor:"auto" + 交接正文缺失或为空）：auto 路径造出的继任者是零上下文的新根会话，交接正文是它得到的全部——空正文的自动换届等于造一个失忆的持钥者（§11.9.6）。本次零建会话、零令牌、零 freeze。\n\n${handoffScaffold()}`,
		};
	}
	...
	if (named.length > 0) {
		return {
			error: `prepare 被拒绝（successor:"auto" + 交接正文缺硬节）：${named.join("、")}。本次零建会话、零令牌、零 freeze——请补齐后重试。\n\n${handoffScaffold()}`,
		};
	}
	const warnings = [];
 ...
 * §11.9.6 事实段's「上下一份交接文档路径」. Read from the directory, not from
 * memory: previous hand-overs may have been written before this activation.
 * @returns `{ path, error }` — `path` is null when none exists, `error` is set
 *   when the directory could not be read at all. */
async function latestHandoffDocument(team, roleName) {
	const dir = blackboardPaths(team).dir;
	try {
		const entries = await readdir(dir);
		const mine = entries.filter((entry) => entry.startsWith(`${HANDOFF_FILE_PREFIX}${roleName}-`) && entry.endsWith(".md")).sort();
		return { path: mine.length === 0 ? null : path.join(dir, mine[mine.length - 1]), error: null };
	} catch (error) {
		if (error !== null && typeof error === "object" && error.code === "ENOENT") return { path: null, error: null };
		return { path: null, error: describeError(error) };
	}
}
...
function createRotation(ctx, policy, extras = {}) {
	...
	async function sweep(options = {}) {
		...
		for (const item of cancelled) {
			...
			if (typeof extras.pluginCreated === "function" && extras.pluginCreated(item.caller) === true) {
				lines.push(`- 本次换届由插件新建的继任者会话 ${item.caller} 仍存活但未认领，可收编或关闭（§11.5：自建继任者必须被点名，否则它会变成一个没人知道的孤儿）。`);
			}
		}
...
function apply(ctx) {
	...
	const rotation = createRotation(ctx, policy, { pluginCreated: (sessionId) => teamSession.hasHandle(sessionId) === true });
